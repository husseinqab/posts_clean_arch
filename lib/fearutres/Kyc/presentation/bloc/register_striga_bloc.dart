  ...
  UserInfoBloc(this.getUserInfo) : super(UserInfoInitial()) {
    on<UserInfoEvent>((event, emit) async {
      if (event is UserInfoEvent) {
        ...
        final eitherFailureOrAirlinesList = await getUserInfo(NoParams());

        eitherFailureOrAirlinesList.fold(
                (failure) => emit(
                UserInfoFailed(message: Helpers.mapFailureToMessage(failure))),
                (userInfoResponse) => emit(UserInfoLoaded(userInfoResponse: userInfoResponse)));
      }
    });
  }
}
